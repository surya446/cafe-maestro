          ...
          {isLoading ? (
            <div className="columns-2 sm:columns-3 gap-4 space-y-4">
              {Array.from({ length: 9 }).map((_, i) => (
                <div
                  key={i}
                  className="break-inside-avoid rounded-xl animate-pulse"
                  style={{ height: `${180 + (i % 3) * 60}px`, background: "#111" }}
                />
              ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
              ...
              <p className="text-base font-medium text-white/30">No gallery images yet</p>
              <p className="text-sm text-white/20">Check back soon — we're adding photos.</p>
            </div>
          ) : (
            <motion.div
              initial="hidden"
